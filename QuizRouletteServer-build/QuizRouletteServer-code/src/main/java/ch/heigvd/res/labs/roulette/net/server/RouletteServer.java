...
/**
 * This class implements the main logic of the RouletteServer. It starts a loop to
 * accept incoming connections on a TCP port. When a new connection request is made
 * by a client, the server spawns a new thread. It instantiates a client handler, to
 * which it delegates the processing of the session (the client handler executes on
 * the thread).
 *
 * It is possible to either create an instance of the RouletteServer by specifying an
 * explicit TCP port (typically, the default port defined in the protocol
 * specification), or to let the OS allocate an ephemeral port. While this is not a
 * common approach for TCP servers (because clients generally want to have a
 * well-known port to establish the communication), it is useful in the context of
 * automated tests (which may execute in parallel and therefore need "isolated"
 * servers).
 *
...
public class RouletteServer {
   ...
   /**
    * Constructor used to create a server that will accept connections on a known TCP
    * port
    *
   ...
   /**
    * Indicates whether the server is accepting connection requests, by checking the
   ...
   /**
    * Requests a server shutdown. This will close the server socket and notify all
    * client workers.
    *
    * @throws IOException
    */
